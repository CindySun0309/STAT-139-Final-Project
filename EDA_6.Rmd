...
check_lm_assumptions <- function(model, data, model_name = "model") {
  ...
  # Set plotting area: 3 rows × 2 columns
  par(mfrow = c(3, 2))
  ...
  lines(lowess(fitted(model), resid(model)), lwd = 2, col = "blue")
  ...
  ## (b2) Histogram of standardized residuals
  ...
  ## (c) Leverage
  h      <- hatvalues(model)
  cut_h  <- 2 * length(coef(model)) / nrow(data)
  plot(h,
       ylab = "Leverage",
       main = paste("Leverage -", model_name))
  ...
  plot(rstud,
       ...
       main = paste("Studentized Residuals -", model_name))
...
# Baseline Algebra II model
check_lm_assumptions(
  model = baseline_alg,
  data  = cleaned_data,
  model_name = "Baseline Model Algebra II"
)

# Baseline English model
check_lm_assumptions(
  model = baseline_eng,
  data  = cleaned_data,
  model_name = "Baseline Model English"
)

# Selected Algebra II model
check_lm_assumptions(
  model = Model_selected_algebra,
  data  = cleaned_data,
  model_name = "Full Model Algebra II"
)

# Selected English model
check_lm_assumptions(
  model = Model_selected_eng,
  data  = cleaned_data,
  model_name = "Full Model English"
)
